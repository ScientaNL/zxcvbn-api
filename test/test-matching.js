...
const adjacency_graphs = require('../src/adjacency_graphs');
...
const check_matches = function(prefix, t, matches, pattern_names, patterns, ijs, props) {
  let i;
  if (typeof pattern_names === "string") {
    ...
    pattern_names = ((() => {
      let asc, end;
      const result = [];
      for (i = 0, end = patterns.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  let is_equal_len_args = pattern_names.length === patterns.length && patterns.length === ijs.length;
  ...
  if (!is_equal_len_args) { throw 'unequal argument lists to check_matches'; }

  let msg = `${prefix}: matches.length == ${patterns.length}`;
  t.equal(matches.length, patterns.length, msg);
  return (() => {
    const result1 = [];
    for (var k = 0, end1 = patterns.length, asc1 = 0 <= end1; asc1 ? k < end1 : k > end1; asc1 ? k++ : k--) {
      let j;
      var match = matches[k];
      ...
      result1.push((() => {
        ...
        for (let prop_name in props) {
          ...
          if (typeof(prop_msg) === 'string') { prop_msg = `'${prop_msg}'`; }
...
test('matching utils', function(t) {
  ...
  for (let value of [
    ...
    const [dividend, divisor] = Array.from(value[0]), remainder = value[1];
...
test('dictionary matching', function(t) {
  ...
  for (let name in test_dicts) {
    ...
    for (word in dict) {
      rank = dict[word];
      if (word === 'motherboard') { continue; } // skip words that contain others
      matches = dm(word);
...
test('omnimatch', function(t) {
  ...
  for (let value of [
    ...
    const pattern_name = value[0], [i, j] = Array.from(value[1]);
    let included = false;
    for (let match of Array.from(matches)) {
      if ((match.i === i) && (match.j === j) && (match.pattern === pattern_name)) { included = true; }
    }
